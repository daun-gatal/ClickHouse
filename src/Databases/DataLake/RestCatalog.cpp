    ...
    TableMetadata & result) const
{
    ...
    {
        auto [parsed_credentials, parsed_endpoint] = getCredentialsAndEndpoint(object, location);
...
ICatalog::CredentialsRefreshCallback RestCatalog::getCredentialsConfigurationCallback()
{
    return [this] (const DB::StorageID & storage_id) -> std::shared_ptr<IStorageCredentials>
    {
        ...
        std::string location;
        {
            if (config_object->has("location"))
            {
                location = config_object->get("location").extract<String>();
                LOG_DEBUG(log, "Location for table {}: {}", table_name, location);
            }
            else
            {
