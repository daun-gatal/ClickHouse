...
static std::exception_ptr runStep(std::function<void()> step, ThreadGroupPtr & thread_group, ProfileEvents::CountersPtr profile_counters_scope)
{
    ThreadGroupSwitcher switcher(thread_group, ThreadName::RUNTIME_DATA, profile_counters_scope, /*allow_existing_group*/ true);
...
void ExceptionKeepingTransform::work()
{
    if (stage == Stage::Start)
    {
        ...
        if (auto exception = runStep([this] { onStart(); }, thread_group, profile_counters_scope))
        {
            stage = Stage::Exception;
            ready_output = true;
    ...
    else if (stage == Stage::Consume || stage == Stage::Generate)
    {
        if (stage == Stage::Consume)
        {
            ...
            if (auto exception = runStep([this] { onConsume(std::move(data.chunk)); }, thread_group, profile_counters_scope))
            {
                stage = Stage::Exception;
                ready_output = true;
                data.exception = exception;
        ...
        {
            GenerateResult res;
            if (auto exception = runStep([this, &res] { res = onGenerate(); }, thread_group, profile_counters_scope))
            {
                stage = Stage::Exception;
                ready_output = true;
                data.exception = exception;
    ...
    {
        GenerateResult res;
        if (auto exception = runStep([this, &res] { res = getRemaining(); }, thread_group, profile_counters_scope))
        {
            stage = Stage::Exception;
            ready_output = true;
            data.exception = exception;
        ...
        else if (auto finish_exception = runStep([this] { onFinish(); }, thread_group, profile_counters_scope))
        {
            stage = Stage::Exception;
            ready_output = true;
...
void ExceptionKeepingTransform::setRuntimeData(ThreadGroupPtr thread_group_, ProfileEvents::CountersPtr profile_counters_scope_)
{
