...
static void threadFunction(
    PullingAsyncPipelineExecutor::Data & data, ThreadGroupPtr thread_group, ProfileEvents::CountersPtr profile_counters_scope, size_t num_threads, bool concurrency_control)
{
    try
    {
        ThreadGroupSwitcher switcher(thread_group, ThreadName::PULLING_ASYNC_EXECUTOR, profile_counters_scope);
...
bool PullingAsyncPipelineExecutor::pull(Chunk & chunk, uint64_t milliseconds)
{
    if (!data)
    {
        ...
        auto func = [&, thread_group = CurrentThread::getGroup(), profile_counters_scope = CurrentThread::getCountersScope()]()
        {
            threadFunction(*data, thread_group, profile_counters_scope, pipeline.getNumThreads(), pipeline.getConcurrencyControl());
        };
