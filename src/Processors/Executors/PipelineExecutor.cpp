...
void PipelineExecutor::spawnThreads(AcquiredSlotPtr slot)
{
    while (cpu_slots)
    {
        ...
        /// Start new thread
        pool->scheduleOrThrowOnError([this, thread_num, thread_group = CurrentThread::getGroup(), profile_counters_scope = CurrentThread::getCountersScope(), my_slot = std::move(slot)]
        {
            ThreadGroupSwitcher switcher(thread_group, ThreadName::QUERY_ASYNC_EXECUTOR, profile_counters_scope);
