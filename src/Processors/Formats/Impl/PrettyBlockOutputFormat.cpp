...
void PrettyBlockOutputFormat::write(Chunk chunk, PortKind port_kind)
{
    ...
    if (mono_block || format_settings.pretty.squash_consecutive_ms)
    {
        if (port_kind == PortKind::Main)
        {
            ...
            {
                thread.emplace([this, thread_group = CurrentThread::getGroup(), profile_counters_scope = CurrentThread::getCountersScope()]
                {
                    ThreadGroupSwitcher switcher(thread_group, ThreadName::PRETTY_WRITER, profile_counters_scope);
