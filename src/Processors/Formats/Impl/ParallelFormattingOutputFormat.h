...
public:
...

    {
        ...
        collector_thread = ThreadFromGlobalPool([this, thread_group = CurrentThread::getGroup(), profile_counters_scope = CurrentThread::getCountersScope()]
        {
            collectorThreadFunction(thread_group, profile_counters_scope);
        });
    }
...
private:
    ...
    {
        pool.scheduleOrThrowOnError([this, thread_group = CurrentThread::getGroup(), profile_counters_scope = CurrentThread::getCountersScope(), ticket_number, first_row_num]
        {
            formatterThreadFunction(ticket_number, first_row_num, thread_group, profile_counters_scope);
        });
    }

    /// Collects all temporary buffers into main WriteBuffer.
    void collectorThreadFunction(const ThreadGroupPtr & thread_group, ProfileEvents::CountersPtr profile_counters_scope);
    ...
    void formatterThreadFunction(size_t current_unit_number, size_t first_row_num, const ThreadGroupPtr & thread_group, ProfileEvents::CountersPtr profile_counters_scope);
