...
void ParquetBlockOutputFormat::startMoreThreadsIfNeeded(const std::unique_lock<std::mutex> &)
{
    ...
    {
        auto job = [this, thread_group = CurrentThread::getGroup(), profile_counters_scope = CurrentThread::getCountersScope()]()
        {
            try
            {
                ThreadGroupSwitcher switcher(thread_group, ThreadName::PARQUET_ENCODER, profile_counters_scope);
