        ...
        else
        {
            ...
            for (size_t part_index = 0; part_index < parts_with_ranges.size(); ++part_index)
            {
                pool.scheduleOrThrow(
                    [&, part_index, thread_group = CurrentThread::getGroup(), profile_counters_scope = CurrentThread::getCountersScope()]
                    {
                        ThreadGroupSwitcher switcher(thread_group, ThreadName::MERGETREE_INDEX, profile_counters_scope);
