...
namespace
{
    DataTypeEnum8::Values getVolumeTypeEnumValues()
    {
        ...
        static constexpr std::array<VolumeType, 3> values = {VolumeType::JBOD, VolumeType::SINGLE_DISK, VolumeType::UNKNOWN};
        for (auto value : values)
            enum_values.emplace_back(std::string(toString(value)), static_cast<Int8>(value));
        return enum_values;
    }
    ...
    {
        DataTypeEnum8::Values enum_values;
        static constexpr std::array<VolumeLoadBalancing, 2> values = {VolumeLoadBalancing::ROUND_ROBIN, VolumeLoadBalancing::LEAST_USED};
        for (auto value : values)
            enum_values.emplace_back(std::string(toString(value)), static_cast<Int8>(value));
        return enum_values;
    }
