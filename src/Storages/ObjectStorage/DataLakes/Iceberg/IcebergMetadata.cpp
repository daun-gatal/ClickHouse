    ...
    ContextPtr local_context) const
{
    ...
    for (const EqualityDeleteObject & delete_file : delete_files)
    {
        auto simple_transform_adder = [&](const SharedHeader & header)
        {
            ...
            {
                auto schema_read_buffer = createReadBuffer(StorageID(), delete_file_object, object_storage, local_context, log);
            ...
            std::unique_ptr<ReadBuffer> data_read_buffer = createReadBuffer(StorageID(), delete_file_object, object_storage, local_context, log);
