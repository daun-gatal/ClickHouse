...
void IcebergPositionDeleteTransform::initializeDeleteSources()
{
    ...
    for (const auto & position_deletes_object : iceberg_object_info->info.position_deletes_objects)
    {
        ...
        {
            std::unique_ptr<ReadBuffer> read_buf_schema = createReadBuffer(StorageID(), object_info, object_storage, context, log);
        ...
        delete_read_buffers.push_back(createReadBuffer(StorageID(), object_info, object_storage, context, log));
