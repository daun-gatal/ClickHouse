...
void AzureStorageParsedArguments::fromAST(ASTs & engine_args, ContextPtr context, bool with_structure)
{
        ...
        if (is_format_arg(fourth_arg))
        {
            ...
            if (PartitionStrategyFactory::isStrategyType(sixth_arg))
            {
                partition_strategy_type
                    = PartitionStrategyFactory::strategyTypeFromString(sixth_arg).value();
            }
            else
            {
                if (with_structure)
    ...
    else if (engine_args.size() == 8)
    {
        ...
        else
        {
            ...
            if (PartitionStrategyFactory::isStrategyType(eighth_arg))
            {
                partition_strategy_type
                    = PartitionStrategyFactory::strategyTypeFromString(eighth_arg).value();
            }
            else
            {
                if (with_structure)
