...
public:
        ...
        , loading_timeout(dictionary.configuration.load_timeout)
    {
        ...
        for (size_t shard = 0; shard < shards; ++shard)
        {
            ...
            try
            {
                pool.scheduleOrThrowOnError([this, shard, thread_group = CurrentThread::getGroup(), profile_counters_scope = CurrentThread::getCountersScope()]
                {
                    ThreadGroupSwitcher switcher(thread_group, ThreadName::HASHED_DICT_LOAD, profile_counters_scope);
