...
private:
    ...
    void startLoading(Info & info, bool forced_to_reload = false, size_t min_id_to_finish_loading_dependencies_ = 1)
    {
        ...
        if (enable_async_loading)
        {
            ...
            try
            {
                thread = ThreadFromGlobalPool{&LoadingDispatcher::doLoading, this, info.name, loading_id, forced_to_reload, min_id_to_finish_loading_dependencies_, true, CurrentThread::getGroup(), CurrentThread::getCountersScope()};
            }
            catch (...)
            {
                cancelLoading(info);
    ...
    void doLoading(const String & name, size_t loading_id, bool forced_to_reload, size_t min_id_to_finish_loading_dependencies_, bool async, ThreadGroupPtr thread_group = {}, ProfileEvents::CountersPtr profile_counters_scope = nullptr)
    {
        ThreadGroupSwitcher switcher(thread_group, ThreadName::EXTERNAL_LOADER, profile_counters_scope);
