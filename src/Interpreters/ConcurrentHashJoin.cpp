    ...
    try
    {
        for (size_t i = 0; i < slots; ++i)
        {
            pool->scheduleOrThrow(
                [&, i, thread_group = CurrentThread::getGroup(), profile_counters_scope = CurrentThread::getCountersScope()]()
                {
                    ThreadGroupSwitcher switcher(thread_group, ThreadName::CONCURRENT_JOIN, profile_counters_scope);
...
ConcurrentHashJoin::~ConcurrentHashJoin()
{
    try
    {
        ...
        for (size_t i = 0; i < slots; ++i)
        {
            ...
            pool->scheduleOrThrow(
                [join = hash_joins[0], i, this, thread_group = CurrentThread::getGroup(), profile_counters_scope = CurrentThread::getCountersScope()]()
                {
                    ThreadGroupSwitcher switcher(thread_group, ThreadName::CONCURRENT_JOIN, profile_counters_scope);
