    ...
    return castColumnString(
        ...
        [&](const auto & data_column)
        {
            ...
            if (index.getType() != Field::Types::String)
                return false;
            detail::MatcherStringConst<DataColumn> matcher{data_column, index.safeGet<String>()};
        ...
        [&](const auto & data_column)
        {
            return castColumnString(
                ...
                [&](const auto & index_column)
                {
                    ...
                    detail::MatcherString<DataColumn, IndexColumn> matcher{data_column, index_column};
                    if (is_key_const)
    ...
    const IColumn & data, const Offsets & offsets, const Field & index, PaddedPODArray<UInt64> & matched_idxs)
{
    return castColumnNumeric(
        ...
        [&](const auto & data_column)
        {
            ...
            detail::MatcherNumberConst<DataType> matcher{data_column.getData(), *index_as_integer};
            executeMatchKeyToIndex(offsets, matched_idxs, matcher);
            return true;
        });
}
        ...
        [&](const auto & data_column)
        {
            return castColumnNumeric(
                ...
                [&](const auto & index_column)
                {
                    ...
                    {
                        detail::MatcherNumber<DataType, IndexType> matcher{data_column.getData(), index_column.getData()};
                        if (is_key_const)
