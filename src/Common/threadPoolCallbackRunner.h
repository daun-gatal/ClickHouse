...
{
    return [my_pool = &pool, thread_name, thread_group = CurrentThread::getGroup(), profile_counters_scope = CurrentThread::getCountersScope()](Callback && callback, Priority priority) mutable -> std::future<Result>
    {
        auto task = std::make_shared<std::packaged_task<Result()>>([thread_group, thread_name, profile_counters_scope, my_callback = std::move(callback)]() mutable -> Result
        {
            ThreadGroupSwitcher switcher(thread_group, thread_name, profile_counters_scope);
...
private:
    ...
    static void executeCallback(std::promise<FunctionResult> & promise, Function && callback, ThreadGroupPtr thread_group, ThreadName thread_name, ProfileEvents::CountersPtr profile_counters_scope)
    {
    ...
    static void executeCallback(std::promise<void> & promise, Function && callback, ThreadGroupPtr thread_group, ThreadName thread_name, ProfileEvents::CountersPtr profile_counters_scope)
    {
        /// Release callback before setting value to the promise to avoid
        /// destruction of captured resources after waitForAllToFinish returns.
        try
...
public:
    ...
    [[nodiscard]] std::shared_ptr<Task> enqueueAndGiveOwnership(Callback && callback, Priority priority = {}, std::optional<uint64_t> wait_microseconds = {})
    {
        ...
        auto task_func = [this, task, thread_group = CurrentThread::getGroup(), profile_counters_scope = CurrentThread::getCountersScope(), my_callback = std::move(callback), promise]() mutable -> void
        {
            TaskState expected = SCHEDULED;
            ...
            executeCallback(*promise, std::move(my_callback), std::move(thread_group), thread_name, profile_counters_scope);
        };
...
public:
    ...
    void initThreadPool(ThreadPool & pool_, size_t max_threads_, ThreadName thread_name_, ThreadGroupPtr thread_group_, ProfileEvents::CountersPtr profile_counters_scope_);
...
private:
    ...
    ProfileEvents::CountersPtr profile_counters_scope;
