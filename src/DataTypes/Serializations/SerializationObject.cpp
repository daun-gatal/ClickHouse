    ...
    SubstreamsDeserializeStatesCache * cache) const
{
    ...
    if (settings.prefixes_deserialization_thread_pool && !structure_state_concrete->sorted_dynamic_paths->empty())
    {
        ...
        for (size_t i = 0; i != num_tasks; ++i)
        {
            ...
            static_cast<void>(settings.prefixes_deserialization_thread_pool->trySchedule([task_ptr = task, thread_group = CurrentThread::getGroup(), profile_counters_scope = CurrentThread::getCountersScope()]()
            {
                ThreadGroupSwitcher switcher(thread_group, ThreadName::PREFIX_READER, profile_counters_scope);
