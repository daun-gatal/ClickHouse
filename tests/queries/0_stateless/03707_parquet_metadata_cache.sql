...
WHERE event_date >= yesterday() AND event_time >= now() - 600 AND (log_comment = '03707-first-test-query') AND (type = 'QueryFinish') AND (current_database = currentDatabase());
...
SELECT
    ...
    ProfileEvents['ParquetMetadataCacheMisses'] AS misses
FROM system.query_log
WHERE event_date >= yesterday() AND event_time >= now() - 600 AND (log_comment = '03707-second-test-query') AND (type = 'QueryFinish') AND (current_database = currentDatabase());
...
SELECT
    ...
    ProfileEvents['ParquetMetadataCacheMisses'] AS misses
FROM system.query_log
WHERE event_date >= yesterday() AND event_time >= now() - 600 AND (log_comment = '03707-third-test-query') AND (type = 'QueryFinish') AND (current_database = currentDatabase());
